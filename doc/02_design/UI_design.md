...
| 3 | **調教師・生産牧場・血統の分析ビューは単体画面とダッシュボード埋め込みで共用**する | 要求 3.1・3.3 / C04〜C06 |
| 4 | 調教師を実データ、牧場・血統・類似馬は骨格／プレースホルダ | 要求 4 |
| 5 | 募集馬は **画面先行**（フォーム状態のみ。永続化は後続） | アーキ 6.3 |
| 6 | 表示は分析済みデータの読取のみ。重い集計は UI で行わない | アーキ Presentation |
| 7 | 強さ・コスパ・複数頭ランキングは **載せない** | 要求 4 / feature flag |
...
| 優先 | 画面名 | 備考 | レイアウト・ビジュアル |
|------|--------|------|------------------------|
| 1（入口） | ホーム | 起動時ランディング。分析・設定への遷移とデータ更新 | [`UI/home.md`](./UI/home.md) |
| 2（本線） | 調教師単体分析 | 分析の本実装・実データ | [`UI/trainer.md`](./UI/trainer.md) |
| 3 | 募集馬分析 | ダッシュボード埋め込みで調教師・生産牧場・血統を表示。各単体画面と共通コンポーネントを共用 | [`UI/horse.md`](./UI/horse.md) |
| 4 | データ更新・設定 | 全画面から到達。詳細設定はここ | [`UI/settings.md`](./UI/settings.md) |
| 5 | 生産牧場単体分析 | 骨格。導線のみ確保 | [`UI/farm.md`](./UI/farm.md) |
| 6 | 血統単体分析 | 骨格。導線のみ確保 | [`UI/pedigree.md`](./UI/pedigree.md) |

```text
ナビに出す（優先順）: ホーム / 調教師 / 募集馬 / 設定 / 生産牧場 / 血統
ナビに出さない: 複数頭比較（§2.9）、強さ・コスパ専用画面
```

複数頭比較はインベントリ行を持たない（将来・文書のみ [`UI/compare.md`](./UI/compare.md)）。
...
6. 分析ダッシュボード: 分析種類切替 ＋ 埋め込み本体  
   - 優先して読ませる: **調教師分析（実データ）** — 調教師単体分析と同一コンポーネント  
   - 枠として載せる: 生産牧場 / 血統 — 各単体分析と同一コンポーネント（骨格）  
   - 枠として載せる: 類似馬（プレースホルダ。単体画面なし）
...
**ホームとの分担:** ホームは「更新の起動・最終更新・短い結果」まで。接続設定・詳細結果サマリは本画面。更新ユースケース自体は同一（C08）。
...
**載せない:** 画面インベントリ・ナビに出さない。ルートを生やす場合も Empty のみ。

将来の載せる要素案: 複数頭リスト → ランキング表 → 1頭選択でダッシュボード。

**配置・見た目:** [`UI/compare.md`](./UI/compare.md)
...
| 条件 | 状態 |
|------|------|
| ホームに分析・設定への遷移とデータ更新がある | 必須 |
| 調教師単体分析の載せる要素 1〜7 が欠けない | 必須 |
| 募集馬分析の必須入力＋ダッシュボードに調教師実データ（単体画面と同一コンポーネント） | 必須 |
| 生産牧場・血統は骨格。本文は募集馬ダッシュボード埋め込みと共用 | 必須 |
| データ更新・設定から更新・接続設定ができる | 必須 |
| 複数頭比較・スコア専用は非表示 | 必須 |

レイアウト・ビジュアルの完了条件は [`UI/README.md`](./UI/README.md) を参照。

### 3.2 再利用マトリクス（単体画面 × ダッシュボード埋め込み）

| 載せる内容 | 単体 | 募集馬分析への埋め込み |
|------------|------|------------------------|
| 調教師分析本文（賞金 / 着回 / 率 / 距離 / 重賞） | 調教師単体（TrainerAnalysisPanel） | `trainer` で同一 |
| 生産牧場分析本文（骨格） | 生産牧場単体 | `farm` で同一 |
| 血統分析本文（骨格） | 血統単体 | `pedigree` で同一 |
| 類似馬 | （単体画面なし） | `similarity` 専用プレースホルダ |

配置上の再利用: [`UI/panels/trainer_analysis.md`](./UI/panels/trainer_analysis.md) / [`UI/panels/farm.md`](./UI/panels/farm.md) / [`UI/panels/pedigree.md`](./UI/panels/pedigree.md) / [`UI/analysis_dashboard.md`](./UI/analysis_dashboard.md)
...
| HomeScreen（ホーム） | C01 + C08 | 遷移ナビ / `updateFromPostgres` / `getUpdateStatus` |
| HorseEntryPanel | C02 | local state（後続 `saveHorse`） |
...
| DataUpdateUI（設定 / ホーム更新） | C08 | `updateFromPostgres` / `getUpdateStatus` |
...
本書は **情報設計のみ** を正本とする。ナビ上の優先は **ホーム → 調教師 → 募集馬 → 設定 → 生産牧場 → 血統**。ホームを起動入口、調教師単体分析を分析本線に「載せる／載せない／ゴール」を固定する。募集馬分析のダッシュボードは調教師・生産牧場・血統を各単体画面と共用する。配置と見た目は [`UI/`](./UI/README.md) の画面・パネル文書に落とし、トークンは `base_desing.md` に委譲する。実装時は情報設計を上書きしない。
