...
| 2 | **分析ダッシュボードを共通枠**とし、分析種類は切替で埋め込む | 要求 3.1・3.3 / アーキ Presentation・6.3 |
| 3 | **調教師・生産牧場・血統の分析ビューは単体画面とダッシュボード埋め込みで共用**する | 要求 3.1・3.4〜3.6 / アーキ 4 |
| 4 | 調教師を実データ、牧場・血統は骨格／プレースホルダ。募集馬ダッシュボードに入力馬の表示（`horse`）を載せる | 要求 4 / アーキ Domain |
| 5 | 募集馬は **画面先行**（フォーム状態のみ。永続化は後続） | アーキ 4（募集馬入力） |
| 6 | 表示は **更新時に永続化した分析結果**の読取のみ。重い集計は UI で行わない | アーキ Presentation・方針 4 |
| 7 | 強さ・コスパ・複数頭ランキングは **載せない** | 要求 4 / アーキ（当面無効） |
| 8 | UI 言語は日本語。開発はブラウザ、製品はデスクトップ（**Tauri**） | 要求 6 / アーキ 5・8.1 |
...
**対応アーキ:** Presentation（シェル・最終更新表示）／手動更新パイプライン起動（アーキ 4・6）。遷移先は各 Screen。
...
**対応アーキ:** 募集馬フォーム ＋ 分析ダッシュボード埋め込み（アーキ 4）。調教師／牧場／血統ビューは各単体画面と共用。
...
**対応アーキ:** 調教師検索 → 分析結果の読取表示（アーキ 4）。表示時に再集計しない。
...
1. 更新実行と進捗（**Sync → Analyze**。アーキ 4・6）
...
**対応アーキ:** 手動更新パイプライン Sync → Analyze → SQLite（アーキ 4・6）。通常操作は SQLite 読取のみ。
...
|-----------|--------------------|--------------------------|
| AppShell | Presentation（ナビ・ヘッダ・最終更新） | 更新状態取得 |
| HomeScreen（ホーム） | Presentation ＋ 更新起動 | ホーム表示 / 手動更新実行 / 更新状態取得 |
| TrainerAnalysisView | Presentation（調教師・読取専用） | 調教師分析表示 |
| HorseEntryPanel | Presentation（募集馬入力） | 募集馬入力・フォーム保持 |
| HorseProfilePanel | Presentation（入力馬の表示） | 募集馬入力（読取表示） |
| AnalysisDashboard | Presentation（分析種類切替・埋め込み） | 分析ダッシュボード切替・埋め込み |
| FarmAnalysisView | Presentation（牧場） | 生産牧場分析表示 |
| PedigreeAnalysisView | Presentation（血統） | 血統分析表示 |
| DataUpdateUI（設定 / ホーム更新） | Presentation（更新 UI） | 手動更新実行 / 更新状態取得 |
| 調教師検索 | Application | 調教師名検索 |
